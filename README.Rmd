...
The function *sample_dose_response* samples from a uniform distribution $\{a, b\}$ and calls the *simulate_dose_respose* function.

Plot four simulated samples to see how they look like and if they resemble your actual data:
...
non_monotone <- dose_responses %>% 
  group_by(draw) %>% 
  summarise(is_non_monotone=non_monotonic(number_surviving))

table(non_monotone$is_non_monotone)

save(dose_responses, file = "data/processed/simulations/nonMonotone_simultns.RData")
